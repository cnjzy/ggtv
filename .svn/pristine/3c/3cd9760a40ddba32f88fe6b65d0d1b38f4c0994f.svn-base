...
public class BladeView extends View {
	...
	public void init(Context context){
		mCharHeight = context.getResources().getDimensionPixelSize(R.dimen.blade_text_size);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < b.length; i++) {
			paint.setColor(Color.RED);//656565
			paint.setTypeface(Typeface.DEFAULT_BOLD);
			paint.setTextSize(mCharHeight);
			paint.setFakeBoldText(true);
			paint.setAntiAlias(true);
			if (i == choose) {
				paint.setColor(Color.parseColor("#007bd7"));
			}
	...
	private void showPopup(int item) {
		if (mPopupWindow == null) {
			...
			mPopupText.setTextColor(Color.parseColor("#007bd7"));
